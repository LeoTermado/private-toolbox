...
from core.validators import is_pdf_filename, filter_nonempty_uploads, validate_upload_count
from . import service
...
def merge():
    ...
    # --- Validation ---
    if not validate_upload_count(files, min_count=2):
        flash("Please select at least 2 PDF files to merge.")
        return redirect(url_for('pdf_merger.index'))

    if any(not is_pdf_filename(f.filename) for f in files):
    ...
    # send_pdf_download reads the file into memory, so no handle is kept on the
    # saved file (the Archive button can move output/ afterwards on Windows).
    # The merged PDF still persists on disk in output/pdf_merger/ for archiving.
    return send_pdf_download(output_path)
